...
export function convertInputs(div, temp, colors) {
    convertTemps(div, temp, colors);
}
function convertTemps(div, temp, color) {
    ...
    if (tempUnit !== "Kelvin") {
        ...
        if (tempUnit === "Celsius") {
            ...
            katex.render(`\\textcolor{${CelciusColor}}{${tempNumber}°C} + \
        \\textcolor{${KelvinColor}}{273K} = \
        \\textcolor{${FinalColor}}{${newTemp}K}`, tempConvertDiv, { throwOnError: false });
        }
        else {
            const FahrenheitColor = color[0];
            let newTemp = ((tempNumber - 32) * 5) / 9 + 273;
            katex.render(`(\\textcolor{${FahrenheitColor}}{${tempNumber}°F} - \\textcolor{${FahrenheitColor}}{32°F}) \
        ...
        \\textcolor{${FinalColor}}{${Number(newTemp.toFixed(2))} K}`, tempConvertDiv, { throwOnError: false });
            removeColor(color);
        }
    }
}
function convertPressure(div, pressure) { }
