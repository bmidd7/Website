import { removeColor } from "./local-modules.js";
...
export function convertTempToKelvin(div, temp, colors) {
    ...
    if (tempUnit === "Celsius") {
        ...
        katex.render(`\\textcolor{${startColor}}{${formatNumber(tempNumber)}\\ ^{\\circ}\\mathrm{C}} + ` +
            `\\textcolor{${middleColor}}{273\\ \\mathrm{K}} = ` +
            `\\textcolor{${endColor}}{${formatNumber(newTemp)}\\ \\mathrm{K}}`, tempConvertDiv, { throwOnError: false });
        return [newTemp, "Kelvin"];
    }
    const newTemp = ((tempNumber - 32) * 5) / 9 + KELVIN_OFFSET;
    katex.render(`(\\textcolor{${startColor}}{${formatNumber(tempNumber)}\\ ^{\\circ}\\mathrm{F}} - ` +
...
export function convertPressureToAtmosphere(div, pressure, colors) {
...
export function convertVolumeToLiters(div, volume, colors) {
    ...
    if (volumeUnit === "Milliliters" || volumeUnit === "Cubic Centimeters") {
        ...
        katex.render(`\\textcolor{${startColor}}{${formatNumber(volumeNumber)}\\ \\mathrm{${shortUnit}}} \\cdot ` +
            `\\frac{\\textcolor{${endColor}}{1\\ \\mathrm{L}}}{\\textcolor{${startColor}}{1000\\ \\mathrm{${shortUnit}}}} = ` +
            `\\textcolor{${endColor}}{${formatNumber(newVolume)}\\ \\mathrm{L}}`, volumeConvertDiv, { throwOnError: false });
        return [newVolume, "Liters"];
    }
    volumeConvertDiv.remove();
    return [volumeNumber, volumeUnit];
}
//# sourceMappingURL=pre-convert-gases.js.map