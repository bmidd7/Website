interface TensorFlowRuntime {
  setBackend(backend: "wasm" | "webgl" | "cpu"): Promise<void>;
  ready(): Promise<void>;
}

interface TeachableMachinePrediction {
  className: string;
  probability: number;
}
...
interface TeachableMachineModel {
  ...
  predict(canvas: HTMLCanvasElement): Promise<TeachableMachinePrediction[]>;
}
...
interface TeachableMachineImageRuntime {
...
declare const tf: TensorFlowRuntime;
declare const tmImage: TeachableMachineImageRuntime;

const startCamButton = document.getElementById("startCamButton") as HTMLButtonElement | null;
...
const MODEL_BASE_URL = "https://teachablemachine.withgoogle.com/models/upiTOButL/";

let model: TeachableMachineModel | null = null;
let webcam: TeachableMachineWebcam | null = null;
...
async function init(): Promise<void> {
  ...
  try {
    ...
    labelContainer = document.getElementById("label-container") as HTMLDivElement | null;
    if (!labelContainer) {
      throw new Error("Missing #label-container.");
    }
