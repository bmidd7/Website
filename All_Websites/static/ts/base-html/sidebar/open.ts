import { sidebar, sidebarOpenButton, sidebarCloseButton } from "../../global-vars.js"; // from grandparent folder
...
function mobileClickOutside(clickLocation: MouseEvent | TouchEvent): void {
  // If sidebar is missing for some reason ...
  if (!sidebar) {
    return; // Don't do anything
  }
  ...
  if (window.innerWidth <= 550) {  // Checks if screen is roughly phone sized in px
    if (
      ...
      clickLocation.target instanceof Element &&
