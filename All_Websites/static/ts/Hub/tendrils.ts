...
function getTendrilPoints(nodeX: number, nodeY: number, index: number, time: number) {
    ...
    const swayBase = Math.min(
        tendril.swayMax,
        Math.max(tendril.swayMin, distance * tendril.swayDistanceRatio),
    );
    const points: Point[] = [];
    ...
    for (let step = 0; step <= segments; step++) {
        ...
        const primaryWave = Math.sin(
            progress * tendril.primaryWaveFrequency
            + time * tendril.primaryWaveSpeed
            + seed * tendril.primaryWaveSeed,
        );
        const secondaryWave = Math.cos(
            progress * tendril.secondaryWaveFrequency
            - time * tendril.secondaryWaveSpeed
            + seed * tendril.secondaryWaveSeed,
        );
        const tipCurl = Math.sin(
            progress * tendril.tipCurlFrequency
            - time * tendril.tipCurlSpeed
            + seed * tendril.tipCurlSeed,
        ) * Math.pow(progress, tendril.tipCurlPower);
        const lateralOffset =
            (primaryWave * swayBase + secondaryWave * swayBase * tendril.secondaryWaveInfluence)
                * envelope
            + tipCurl * swayBase * tendril.tipCurlInfluence;
        const axialOffset =
            Math.sin(
                progress * tendril.axialWaveFrequency
                + time * tendril.axialWaveSpeed
                + seed * tendril.axialWaveSeed,
            ) * swayBase * tendril.axialWaveInfluence * envelope;
...
function updateProjectOrbitRadii(viewportBaseSize: number) {
