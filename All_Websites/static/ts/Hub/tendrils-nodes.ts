import {
    ...
    type NodeElType,
    type Project,
} from "./tendrils-config.js";

let activeDrag: DragState | null = null;

function getIdleOffset(node: NodeElType, index: number, time: number) {
    ...
    const limitRadius = node._maxDragDistance
        ? Math.max(
            idleMotion.minRadius,
            Math.min(baseRadius, node._maxDragDistance * idleMotion.maxDragRatio),
        )
        : baseRadius;
    const x =
        Math.sin(time * idleMotion.xPrimarySpeed + seed * idleMotion.seedXPrimary)
            * limitRadius
    ...
    const y =
        Math.cos(time * idleMotion.yPrimarySpeed + seed * idleMotion.seedYPrimary)
            * limitRadius
...
export function setNodePosition(node: NodeElType, x: number, y: number) {
...
export function setNodeTarget(node: NodeElType, x: number, y: number) {
    node._targetX = x;
    node._targetY = y;
}

function suppressNextClick(node: NodeElType) {
...
export function setMaxDragDistances(nodeEls: NodeElType[]) {
    nodeEls.forEach((node, i) => {
        const nodeInitialX = node._initialX;
        const nodeInitialY = node._initialY;

        if (nodeInitialX === undefined || nodeInitialY === undefined) return;

        let nearestNodeDistance = Infinity;

        nodeEls.forEach((otherNode, j) => {
            if (i === j || otherNode._initialX === undefined || otherNode._initialY === undefined) return;

            const dx = otherNode._initialX - nodeInitialX;
            const dy = otherNode._initialY - nodeInitialY;
...
export function clampToNodeDragLimit(node: NodeElType, x: number, y: number) {
    if (
        node._initialX === undefined ||
...
export function createProjectNodes(projects: Project[]) {
    ...
    const nodeEls: NodeElType[] = [];

    projects.forEach((project) => {
        const wrapper = document.createElement("div") as NodeElType;
        ...
        wrapper.addEventListener("pointerdown", (event) => {
            if (wrapper._x === undefined || wrapper._y === undefined) return;
        ...
        wrapper.addEventListener("pointermove", (event) => {
            if (!activeDrag || activeDrag.node !== wrapper || activeDrag.pointerId !== event.pointerId) return;
...
export function updateNodeMotion(nodeEls: NodeElType[]) {
