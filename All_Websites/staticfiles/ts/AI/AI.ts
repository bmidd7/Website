...
const FALLBACK_MESSAGE: StoredMessage = {
  role: "assistant",
...
const requiredClearHistoryButton = requireElement(
  clearHistoryButton,
  "Missing #clear-history.",
);
...
function setRawText(element: HTMLElement, text: string) {
...
function setMessageFlags(
  element: HTMLElement,
  options: Required<Pick<MessageOptions, "saveToHistory" | "includeInConversation" | "editable">>,
) {
...
function resizeEditor(textarea: HTMLTextAreaElement, maxHeight = 320) {
...
function createActionButton(label: string, className: string) {
...
function updateEditButtonsDisabledState() {
  ...
  requiredChatLog.querySelectorAll<HTMLButtonElement>(".message-edit-button").forEach((button) => {
    button.disabled = isBusy;
  });
}

function removeMessagesAfter(messageElement: HTMLElement) {
  let nextMessage = messageElement.nextElementSibling as HTMLElement | null;
  let removedAny = false;

  while (nextMessage) {
    const current = nextMessage;
    nextMessage = current.nextElementSibling as HTMLElement | null;
    current.remove();
    removedAny = true;
  }
...
function renderMessageActions(messageElement: HTMLElement) {
  let actions = messageElement.querySelector(".message-actions") as HTMLDivElement | null;
  if (!actions) {
...
function cancelEditingMessage(messageElement: HTMLElement, editingActions: HTMLElement) {
  const body = messageElement.querySelector(".message-body");
  if (!body) {
    return;
  }
...
function startEditingMessage(messageElement: HTMLElement) {
  if (messageElement.dataset.editable !== "true" || requiredChatSubmit.disabled) {
    return;
  }

  const existingEditor = requiredChatLog.querySelector<HTMLElement>(".message.editing");
  if (existingEditor && existingEditor !== messageElement) {
    const cancelButton = existingEditor.querySelector<HTMLButtonElement>(".message-cancel-button");
    cancelButton?.click();
  }
...
function saveStoredMessages() {
  const messages = Array.from(
    requiredChatLog.querySelectorAll<HTMLElement>('.message[data-save-to-history="true"]'),
  ).map((message): StoredMessage => {
    const role: MessageRole = message.classList.contains("user") ? "user" : "assistant";
...
function removeFallbackMessage() {
  requiredChatLog
    .querySelectorAll<HTMLElement>('.message[data-fallback="true"]')
...
function getConversationMessages() {
  return Array.from(
    requiredChatLog.querySelectorAll<HTMLElement>('.message[data-include-in-conversation="true"]'),
  ).map((message) => ({
...
async function submitMessage() {
  ...
  try {
    ...
    const data = await parseJsonResponse<ChatSuccessResponse & ErrorResponse>(response);
