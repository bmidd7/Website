...
type ProjectDefinition = Omit<Project, "orbitRadius" | "orbitRatio">;

function getViewportBaseSize(): number {
  return Math.min(window.innerWidth, window.innerHeight);
}

function getOrbitRatio(minViewportRatio: number, maxViewportRatio: number): number {
...
export const PROJECTS: Project[] = HUB_CONFIG.projects.map((project) => {
  const orbitRatio = getOrbitRatio(
    HUB_CONFIG.orbitRadius.minViewportRatio,
    HUB_CONFIG.orbitRadius.maxViewportRatio,
  );
...
export function updateProjectOrbitRadii(projects: Project[]): void {
