import { sidebarHandle, sidebar } from "../../global-vars.js"; // from grandparent folder
...
if (sidebarHandle && sidebar) {
  ...
  sidebarHandle.addEventListener("pointerdown", (clickLocation) => {
    ...
    activeSidebar.style.transition = "none";
    activeSidebar.classList.add("open"); // If user drags the bar, sidebar should be considered open

    startingMouseX = clickLocation.clientX; // X pos of pointer
    startingWidth = activeSidebar.offsetWidth; // Just get current width of sidebar
...
document.addEventListener("pointermove", (clickLocation) => {
  ...
  const mouseChange = clickLocation.clientX - startingMouseX;
